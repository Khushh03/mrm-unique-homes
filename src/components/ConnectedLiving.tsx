...
import { Compass, Train, ShoppingBag, Landmark as ArtsIcon, Pin, Car, Eye, MapPin, Navigation } from 'lucide-react';
import { LANDMARKS_DATA } from '../data';
import { Landmark } from '../types';

export function ConnectedLiving() {
  const [filterType, setFilterType] = useState<'all' | 'transit' | 'retail' | 'arts'>('all');
  ...
  return (
    <section id="connected-living" className="max-w-7xl mx-auto px-6 md:px-20 py-24 select-none">
            ...
            {[
              ...
              return (
                <div
                  ...
                  onClick={() => setFilterType(isActive ? 'all' : (cat.id as any))}
                ...
                >
                  <div className={`p-2.5 rounded-lg shrink-0 ${
                    isActive ? 'bg-black text-white' : 'bg-gray-50 text-gray-400 border border-gray-100 shadow-sm'
                  }`}>
                  ...
                  <div>
                    <h4 className="font-serif text-base font-bold text-black flex items-center gap-2">
                      {cat.title}
                      {isActive && <span className="text-[8px] uppercase tracking-widest bg-black text-white px-1.5 py-0.5 rounded font-bold">Active Filter</span>}
                    </h4>
        ...
        <div className="lg:col-span-7">
          <div className="flex flex-col space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs">
              <span className="font-sans text-[10px] font-semibold uppercase tracking-widest text-gray-400 mr-2">Filter Pins:</span>
              <button
                id="filter-map-all"
                onClick={() => { setFilterType('all'); setSelectedLandmark(LANDMARKS_DATA[0]); }}
              ...
              {(['transit', 'retail', 'arts'] as const).map((type) => (
                <button
                  ...
                  onClick={() => {
                    setFilterType(type);
                    const firstOfCategory = LANDMARKS_DATA.find((l) => l.type === type);
            ...
            >
              ...
              <div className="absolute border border-soft-sage/5 rounded-full w-[150px] h-[150px] animate-pulse pointer-events-none" style={{ animationDuration: '8s' }} />
              ...
              {/* Grid axes labels */}
              <div className="absolute left-4 top-4 font-mono text-[9px] text-soft-sage/40">LAT: 40°44'21" N</div>
              <div className="absolute right-4 bottom-4 font-mono text-[9px] text-soft-sage/40">LNG: 74°00'12" W</div>
              ...
              >
                <div className="w-12 h-12 rounded-full bg-primary/20 flex items-center justify-center animate-ping absolute" style={{ animationDuration: '3s' }} />
              ...
              {filteredLandmarks.map((landmark) => {
                ...
                return (
                  <button
                    ...
                    style={{ left: `${landmark.pxX}%`, top: `${landmark.pxY}%` }}
                  >
                    {/* Ring highlight */}
                    <div className={`p-2 rounded-full border-2 transition-all shadow-md ${
                      isSelected 
              ...
              <div className="absolute bottom-4 left-4 right-4 bg-background/95 backdrop-blur-md p-4 rounded-xl shadow-lg border border-outline-variant/20 z-20 flex gap-4 items-start select-none">
                {selectedLandmark ? (
                  <>
                    <div className={`p-2.5 rounded-lg border shrink-0 ${getCategoryColor(selectedLandmark.type)}`}>
                      {(() => {
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex justify-between items-start gap-2">
                        ...
                        <span className="shrink-0 flex items-center gap-1 font-sans text-[10px] font-semibold text-soft-sage bg-surface-container/50 px-2 py-0.5 rounded-full uppercase">
                          <Navigation className="h-2.5 w-2.5 uppercase text-primary transform rotate-45" /> {selectedLandmark.distance}
                        </span>
                      </div>
                  ...
                  <p className="text-center w-full font-sans text-xs text-on-surface-variant/70 italic py-2">
                    Click any point beacon on the map grid to view locality details.
                  </p>
                )}
              </div>
            </div>
            ...
            <p className="text-center font-sans text-[11px] text-on-surface-variant/60 flex items-center justify-center gap-1">
              <Eye className="h-3 w-3" /> Selected beacon represents distance relative to <strong>MRM Unique Homes</strong> sanctuary.
            </p>
          </div>
        </div>
