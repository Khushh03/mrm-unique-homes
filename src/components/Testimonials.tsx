...
const GOOGLE_REVIEW_URL = 'https://www.google.com/search?q=MRM+Unique+Homes+PG+For+Girls+II+Anand+Niketan+Reviews';
...
const reviews = [
  ...
  },
  {
    ...
    review:
      'Environment is friendly & Best PG for Girls.',
      ...
      <div className="relative max-w-7xl mx-auto px-6 md:px-20">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="mt-4 text-sm sm:text-base text-on-surface-variant max-w-2xl mx-auto leading-7">
            Trusted by girls seeking a premium, secure and comfortable stay in New Delhi.
          </p>
          <p className="mt-3 text-xs uppercase tracking-[0.28em] text-slate-400">
            Based on Google reviews for MRM Unique Homes PG For Girls II, Anand Niketan.
          </p>
        </div>
              ...
              {reviews.map((review, index) => (
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                      <div className="h-16 w-16 rounded-full overflow-hidden ring-2 ring-primary/20">
                        <img src={review.image} alt={review.name} className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                      </div>
                      <div>
                        <h3 className="text-lg font-semibold text-primary">{review.name}</h3>
                  ...
                  <div className="mt-6 flex items-center justify-between gap-3 text-xs uppercase tracking-[0.18em] text-on-surface-variant">
                    <span className="rounded-full bg-slate-100 px-3 py-2 text-slate-600">{review.highlight}</span>
                    <span className="inline-flex items-center gap-2 rounded-full bg-slate-950 px-3 py-2 text-white">
                      <svg viewBox="0 0 24 24" className="h-4 w-4" aria-hidden="true">
                        <path fill="#4285F4" d="M12 11.5v3.5h5.1c-.22 1.14-.9 2.1-1.92 2.75l3.1 2.4C20.58 19.4 22 16.98 22 14c0-1.1-.1-2.17-.28-3.21H12z" />
                        <path fill="#34A853" d="M6.2 14.48c-.18-.54-.28-1.12-.28-1.72s.1-1.18.28-1.72L3.1 8.64A8.98 8.98 0 0 0 2 12c0 1.51.37 2.94 1.1 4.24l3.1-1.76z" />
                        <path fill="#FBBC05" d="M12 6.5c1.33 0 2.52.46 3.47 1.22l2.6-2.6C16.98 3.59 14.69 2.5 12 2.5A9 9 0 0 0 3.1 8.64l3.1 2.74A5.99 5.99 0 0 1 12 6.5z" />
                        <path fill="#EA4335" d="M6.2 9.52L3.1 6.78A8.98 8.98 0 0 0 2 12c0 1.51.37 2.94 1.1 4.24l3.1-1.76c-.18-.54-.28-1.12-.28-1.72s.1-1.18.28-1.72z" />
                      </svg>
                      Google
                    </span>
                  </div>
            ...
            {stats.map((stat, idx) => (
              <div key={stat.id} className="rounded-3xl bg-slate-50 p-5">
                <p className="text-3xl font-semibold tracking-tight text-primary">
                  {stat.id === 'stat-rating' ? rating.toFixed(1) : counters[idx].toLocaleString()}{stat.suffix}
                </p>
        ...
        <div className="mt-16 rounded-[2rem] bg-slate-950 px-8 py-12 text-center text-white shadow-2xl shadow-slate-950/20">
          <div className="mx-auto max-w-2xl">
            <p className="text-sm uppercase tracking-[0.35em] text-slate-400">Share Your Experience With Us</p>
            <h3 className="mt-4 text-3xl font-semibold tracking-tight">Join our community of joyful residents.</h3>
            <p className="mt-4 text-sm leading-7 text-slate-300">
              Preview verified feedback from MRM Unique Homes PG For Girls II and explore why residents choose this premium Anand Niketan address.
            </p>
            <button
              type="button"
